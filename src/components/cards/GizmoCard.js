...
import { createGizmoFavorite, deleteGizmoFavorite } from "../../api/dataAccess";
...
export const GizmoCard = ({
  ...
  const favoriteOrUnfavorite = async (e) => {
    e.preventDefault();
    if (favorite) {
      setFavorite(!favorite);
      await deleteGizmoFavorite(favoriteId);
    } else {
      setFavorite(!favorite);
      const favoriteData = {
      ...
      const response = await createGizmoFavorite(favoriteData);
    }
  };
        ...
        {variant === "publicCard" ? (
          <>
            ...
            {favorite ? (
              <div
                onClick={(click) => {
                  favoriteOrUnfavorite(click);
                }}
                className="bg-white shadow-sm shadow-black opacity-60 group-hover:opacity-100 absolute top-2 left-2 p-2 rounded-full cursor-pointer w-12"
              >
                <svg
